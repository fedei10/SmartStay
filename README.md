...
- [DeepWiki Project Overview Full Text](#deepwiki-project-overview-full-text)
- [DeepWiki Documentation Index](#deepwiki-documentation-index)
- [Main Features](#main-features)
- [High-Level Architecture](#high-level-architecture)
- [Tech Stack](#tech-stack)
- [Core Booking Workflows](#core-booking-workflows)
  - [Hotel Booking Workflow](#hotel-booking-workflow)
  - [Flight Booking Workflow](#flight-booking-workflow)
- [AI Agent System](#ai-agent-system)
- [Backend Architecture](#backend-architecture)
- [Frontend Architecture](#frontend-architecture)
- [LiteAPI Integration](#liteapi-integration)
...
> Source: [SmartStay / Ttrip — Project Overview on DeepWiki](https://deepwiki.com/fedei10/SmartStay/1-smartstay-%28ttrip%29-project-overview)
...
2. **Stateless API with Stateful Agents:** The FastAPI layer remains largely stateless, while the agent's state is persisted across turns using an `AsyncPostgresSaver` checkpointer or `MemorySaver`.
...
## DeepWiki Documentation Index

The complete DeepWiki documentation for this repository is available through the badge below and the detailed index. Use this section as the documentation map for the GitHub README.

[![Ask DeepWiki](https://deepwiki.com/badge.svg)](https://deepwiki.com/fedei10/SmartStay)

### 1. Project Overview and Setup

| Section | DeepWiki Page | Purpose |
|---|---|---|
| 1 | [SmartStay / Ttrip Project Overview](https://deepwiki.com/fedei10/SmartStay/1-smartstay-%28ttrip%29-project-overview#smartstay-ttrip-project-overview) | General project description, source files, architecture summary, stack, and external integrations. |
| 1.1 | [Getting Started and Local Development](https://deepwiki.com/fedei10/SmartStay/1.1-getting-started-and-local-development#getting-started-local-development) | Local setup, Docker Compose services, environment preparation, and development tooling. |
| 1.2 | [Configuration and Settings Reference](https://deepwiki.com/fedei10/SmartStay/1.2-configuration-and-settings-reference#configuration-settings-reference) | Pydantic settings, environment variables, provider configuration, Redis, Elasticsearch, security, and operations toggles. |

### 2. Backend Architecture

| Section | DeepWiki Page | Purpose |
|---|---|---|
| 2 | [Backend Architecture](https://deepwiki.com/fedei10/SmartStay/2-backend-architecture#backend-architecture) | FastAPI backend structure, lifecycle initialization, layered architecture, service orchestration, and dependency flow. |
| 2.1 | [API Layer — Routes and Schemas](https://deepwiki.com/fedei10/SmartStay/2.1-api-layer-routes-and-schemas#api-layer-routes-schemas) | API routes, request/response schemas, validation models, and endpoint organization. |
| 2.2 | [Core Middleware, Metrics, and Security](https://deepwiki.com/fedei10/SmartStay/2.2-core-middleware-metrics-and-security#core-middleware-metrics-security) | Middleware, request context, CORS, Prometheus metrics, rate limiting, logging, and security controls. |
| 2.3 | [Database Layer — Models and Repositories](https://deepwiki.com/fedei10/SmartStay/2.3-database-layer-models-and-repositories#database-layer-models-repositories) | Database models, repository pattern, SQLAlchemy async usage, persistence boundaries, and booking data storage. |

### 3. AI Agent System

| Section | DeepWiki Page | Purpose |
|---|---|---|
| 3 | [AI Agent System](https://deepwiki.com/fedei10/SmartStay/3-ai-agent-system#ai-agent-system) | Multi-agent architecture, orchestration goals, travel assistant behavior, and agent responsibilities. |
| 3.1 | [LangGraph Booking Graph — Nodes and Routing](https://deepwiki.com/fedei10/SmartStay/3.1-langgraph-booking-graph-nodes-and-routing#langgraph-booking-graph-nodes-routing) | LangGraph nodes, routing decisions, graph flow, and booking workflow state transitions. |
| 3.2 | [BookingState and Agent Schemas](https://deepwiki.com/fedei10/SmartStay/3.2-bookingstate-and-agent-schemas#bookingstate-agent-schemas) | BookingState structure, agent schemas, travel slots, selected offers, prebook data, and final confirmation state. |
| 3.3 | [LLM Provider Integration and Dependencies](https://deepwiki.com/fedei10/SmartStay/3.3-llm-provider-integration-and-dependencies#llm-provider-integration-dependencies) | Gemini, Groq, SwiftRouter-compatible provider setup, fallback logic, and dependency injection for AI calls. |
| 3.4 | [Prompt Templates and Orchestrator Configuration](https://deepwiki.com/fedei10/SmartStay/3.4-prompt-templates-and-orchestrator-configuration#prompt-templates-orchestrator-configuration) | Prompt templates, orchestration configuration, intent extraction, clarification behavior, and tool-use instructions. |
| 3.5 | [MCP Tools and LiteAPI Tool Provider](https://deepwiki.com/fedei10/SmartStay/3.5-mcp-tools-and-liteapi-tool-provider#mcp-tools-liteapi-tool-provider) | MCP tool provider, LiteAPI tools exposed to agents, tool filtering, timeouts, and tool execution patterns. |

### 4. External Services and Integrations

| Section | DeepWiki Page | Purpose |
|---|---|---|
| 4 | [External Services and Integrations](https://deepwiki.com/fedei10/SmartStay/4-external-services-and-integrations#external-services-integrations) | Overview of external providers, integration boundaries, and service responsibilities. |
| 4.1 | [LiteAPI — Hotels and Flights Services](https://deepwiki.com/fedei10/SmartStay/4.1-liteapi-hotels-and-flights-services#liteapi-hotels-flights-services) | LiteAPI service clients, hotel search, flight search, prebooking, booking, cancellation, and provider request handling. |
| 4.2 | [Redis — Conversation Memory and Session State](https://deepwiki.com/fedei10/SmartStay/4.2-redis-conversation-memory-and-session-state#redis-conversation-memory-session-state) | Redis-backed memory, session state, short-term conversation context, and payment/session continuity. |
| 4.3 | [Elasticsearch — Knowledge Retrieval / RAG](https://deepwiki.com/fedei10/SmartStay/4.3-elasticsearch-knowledge-retrieval-%28rag%29#elasticsearch-knowledge-retrieval-rag) | Knowledge retrieval, RAG index configuration, orchestration context injection, and retrieval limits. |

### 5. Payment Flow

| Section | DeepWiki Page | Purpose |
|---|---|---|
| 5 | [Payment Flow](https://deepwiki.com/fedei10/SmartStay/5-payment-flow#payment-flow) | LiteAPI Payment SDK flow, prebook payment metadata, payment redirect, internal sentinel message, and final booking confirmation. |

### 6. Frontend Application

| Section | DeepWiki Page | Purpose |
|---|---|---|
| 6 | [Frontend Application](https://deepwiki.com/fedei10/SmartStay/6-frontend-application#frontend-application) | Next.js frontend structure, app router, authenticated pages, chat UI, and payment UI. |
| 6.1 | [Chat Interface and Message Rendering](https://deepwiki.com/fedei10/SmartStay/6.1-chat-interface-and-message-rendering#chat-interface-message-rendering) | Chat components, user/assistant message rendering, hotel and flight result rendering, and UI interaction patterns. |
| 6.2 | [Landing Page and Layout Components](https://deepwiki.com/fedei10/SmartStay/6.2-landing-page-and-layout-components#landing-page-layout-components) | Landing page, layout components, header/sidebar design, and reusable frontend UI pieces. |
| 6.3 | [Frontend API Proxy and Type Definitions](https://deepwiki.com/fedei10/SmartStay/6.3-frontend-api-proxy-and-type-definitions#frontend-api-proxy-type-definitions) | Next.js API proxy, backend communication, typed frontend models, and response normalization. |

### 7. Infrastructure and Deployment

| Section | DeepWiki Page | Purpose |
|---|---|---|
| 7 | [Infrastructure and Deployment](https://deepwiki.com/fedei10/SmartStay/7-infrastructure-and-deployment#infrastructure-deployment) | Deployment architecture, service layout, environment strategy, and production-readiness concerns. |
| 7.1 | [Docker and Docker Compose](https://deepwiki.com/fedei10/SmartStay/7.1-docker-and-docker-compose#docker-docker-compose) | Dockerfiles, compose services, ports, volumes, profiles, and local container orchestration. |
| 7.2 | [Kubernetes and Terraform](https://deepwiki.com/fedei10/SmartStay/7.2-kubernetes-and-terraform#kubernetes-terraform) | Kubernetes manifests, Terraform modules, infrastructure-as-code plans, and deployment evolution. |

### 8. Testing

| Section | DeepWiki Page | Purpose |
|---|---|---|
| 8 | [Testing](https://deepwiki.com/fedei10/SmartStay/8-testing#testing) | Overall testing strategy and test organization. |
| 8.1 | [Unit Tests](https://deepwiki.com/fedei10/SmartStay/8.1-unit-tests#unit-tests) | Unit-level tests for isolated services, functions, schemas, and agent components. |
| 8.2 | [Integration Tests](https://deepwiki.com/fedei10/SmartStay/8.2-integration-tests#integration-tests) | Integration tests for API routes, external-service wrappers, booking flows, and persistence. |

### 9. Glossary

| Section | DeepWiki Page | Purpose |
|---|---|---|
| 9 | [Glossary](https://deepwiki.com/fedei10/SmartStay/9-glossary#glossary) | Key project terms, acronyms, and concepts used across the SmartStay / Ttrip documentation. |
